...
class ForecastsManager {
    constructor() { this.forecastsData = {}; this.selectedZone = null; }

    async loadForecasts(silent = false) {
        try {
            if (!silent) {
            ...
            if ((response.success === true || response.status === 'success') && response.data) {
                ...
                if (!silent) this.renderZoneSelector();
                else this.renderZoneSelector(); // Igual render después
            ...
            if (!silent) this.showError('Error al cargar los pronósticos: ' + error.message);
            throw error;
        }
    }
    ...
    renderForecastDetails(zone) {
        ...
        // Normalizar y filtrar pronósticos (evitar fechas pasadas)
        const todayLocal = new Date();
        todayLocal.setHours(0,0,0,0);
        const normalizedForecasts = (zoneData.pronostico || [])
            .map(f => ({...f, _date: this.parseForecastDate(f.fecha)}))
            .filter(f => f._date && f._date >= todayLocal)
            .sort((a,b) => a._date - b._date);
        let html = `
            <div class="forecast-details">
                <div class="forecast-header">
                    <button class="back-btn" onclick="forecastsManager.goBackToSelector()"><i class="fas fa-arrow-left"></i> Volver</button>
                    <h2>${zoneName}</h2>
        ...
        normalizedForecasts.forEach((forecast, index) => {
            const label = this.labelForDate(index, forecast._date, todayLocal);
            html += this.createDayElement(forecast, label, index);
        });
    ...
    labelForDate(position, dateObj, today) {
        if (!dateObj) return 'Día';
        const diffDays = Math.round((dateObj - today) / 86400000);
        if (diffDays === 0) return 'Hoy';
        if (diffDays === 1) return 'Mañana';
        return this.getDayName(dateObj);
    }

    parseForecastDate(raw) {
        if (!raw) return null;
        // Intentar formatos comunes: YYYY-MM-DD, DD/MM/YYYY, DD-MM-YYYY
        let d=null; if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) { const [y,m,da]=raw.split('-').map(Number); d=new Date(y,m-1,da); }
        else if (/^\d{2}[\/\-]\d{2}[\/\-]\d{4}$/.test(raw)) { const part=raw.includes('/')?raw.split('/'):raw.split('-'); const [dd,mm,yy]=part.map(Number); d=new Date(yy,mm-1,dd); }
        else { const tmp=new Date(raw); if(!isNaN(tmp)) d= new Date(tmp.getFullYear(), tmp.getMonth(), tmp.getDate()); }
        if (d && !isNaN(d)) { d.setHours(0,0,0,0); return d; }
        return null;
    }
}
